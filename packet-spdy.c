...
static gboolean match_strval(guint32 string, const value_string matches[3]);
...
static spdy_conv_t * get_or_create_spdy_conversation_data(packet_info *pinfo) {
  ...
  if (!conv_data) {
    ...
    conv_data = wmem_new(wmem_packet_scope(), spdy_conv_t);

    conv_data->streams = NULL;
    if (spdy_decompress_headers) {
      conv_data->rqst_decompressor = wmem_new(wmem_packet_scope(), z_stream);
      conv_data->rply_decompressor = wmem_new(wmem_packet_scope(), z_stream);
...
static void spdy_save_stream_info(spdy_conv_t *conv_data,
  ...
  si = wmem_new(wmem_packet_scope(), spdy_stream_info_t);
...
static void spdy_add_data_chunk(spdy_conv_t *conv_data,
  ...
  if (si == NULL) {
    ...
    spdy_data_frame_t *df = wmem_new(wmem_packet_scope(), spdy_data_frame_t);
    df->data = data;
    df->length = length;
    df->framenum = frame;
...
static spdy_stream_info_t* spdy_assemble_data_frames(spdy_conv_t *conv_data,
  ...
  if (si->assembled_data == NULL) {
     ...
     */
    while (dflist != NULL) {
      df = (spdy_data_frame_t *)dflist->data;
      datalen += df->length;
      dflist = g_slist_next(dflist);
    }
    if (datalen != 0) {
      data = (guint8 *)wmem_alloc(wmem_packet_scope(), datalen);
...
static void spdy_discard_data_frames(spdy_stream_info_t *si) {
  ...
  while (dflist != NULL) {
    df = (spdy_data_frame_t *)dflist->data;
    if ((spdy_data_frame_t *)df->data != NULL) {
      g_free(df->data);
      df->data = NULL;
    }
    dflist = g_slist_next(dflist);
...
static int dissect_spdy_data_payload(tvbuff_t *tvb,
  ...
  if (frame->length != 0 || num_data_frames != 0) {
     ...
     */
    if (frame->length != 0) {
      next_tvb = tvb_new_subset_length_caplen(tvb, offset, frame->length, frame->length);
    ...
    if (have_entire_body && si->content_encoding != NULL &&
       ...
       */
      e_ti = proto_tree_add_text_internal(top_level_tree, data_tvb,
                                 0, tvb_captured_length(data_tvb),
      ...
      if (si->num_data_frames > 1) {
        ...
        spdy_data_frame_t *df;
        guint32 framenum;
        ce_ti = proto_tree_add_text_internal(e_tree, data_tvb, 0,
                                    tvb_captured_length(data_tvb),
    ...
    if (handle == NULL && have_entire_body && si->content_type != NULL &&
      ...
      if (si->content_type_parameters) {
        pinfo->private_table[proto_spdy] = wmem_strdup(wmem_packet_scope(), si->content_type_parameters);
      } else {
        pinfo->private_table[proto_spdy] = NULL;
      }
      /*
...
static spdy_header_info_t* spdy_save_header_block(packet_info *pinfo,
                                                  ...
                                                  guint8 *header,
                                                  guint length) {
  GSList *filist = (GSList *)p_get_proto_data(wmem_packet_scope(), pinfo, proto_spdy, pinfo->fd);
  spdy_header_info_t *header_info = wmem_new(wmem_file_scope(), spdy_header_info_t);
  if (filist != NULL)
    spdy_p_remove_proto_data(pinfo->fd, proto_spdy);
  ...
  p_add_proto_data(wmem_file_scope(), pinfo, pinfo->fd, proto_spdy, filist);
  return header_info;
...
static spdy_header_info_t* spdy_find_saved_header_block(packet_info *pinfo,
                                                        guint32 stream_id,
                                                        guint16 frame_type) {
  GSList *filist = (GSList *)p_get_proto_data(wmem_file_scope(), pinfo, pinfo->fd, proto_spdy);
  while (filist != NULL) {
...
static int dissect_spdy_header_payload(
  ...
  if (!spdy_decompress_headers) {
    ...
    if (header_info == NULL) {
      ...
      header_info = spdy_save_header_block(pinfo,
                                           stream_id,
                                           frame->type,
                                           uncomp_ptr,
                                           uncomp_length);
  ...
  while (num_headers--) {
    ...
    if (tvb_captured_length_remaining(header_tvb, hdr_offset) < 4) {
    ...
    hdr_offset += 4;
    if (tvb_captured_length_remaining(header_tvb, hdr_offset) < header_name_length) {
      expert_add_info_format(pinfo, frame_tree, PI_MALFORMED, PI_ERROR,
                             "Not enough frame data for header name.");
      break;
    }
    header_name = (gchar *)tvb_new_subset_length(header_tvb,
                                                    hdr_offset,
                                                    header_name_length);
    ...
    hdr_offset += 4;
    if (tvb_captured_length_remaining(header_tvb, hdr_offset) < header_value_length) {
      expert_add_info_format(pinfo, frame_tree, PI_MALFORMED, PI_ERROR,
                             "Not enough frame data for header value.");
      break;
    }
    header_value = (gchar *)tvb_new_subset_length(header_tvb,
                                                     hdr_offset,
                                                     header_value_length);
...
static gboolean match_strval(guint32 string, const value_string matches[3]) { // TODO: implement
  return NULL;
}
...
int dissect_spdy_frame(tvbuff_t *tvb,
   ...
   */
  if ((guint)tvb_captured_length_remaining(tvb, offset) < frame.length) {
    expert_add_info_format(pinfo, tree, PI_MALFORMED, PI_ERROR,
                           "Not enough frame data: %d vs. %d",
                           frame.length, tvb_captured_length_remaining(tvb, offset));
    return -1;
  }
 ...
 */
static gboolean dissect_spdy_heur(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
                                  void *data _U_) {
  ...
  /* Attempt dissection. */
  if (dissect_spdy(tvb, pinfo, tree, data) != 0) {
    return TRUE;
  }
...
void proto_register_spdy(void) {
  ...
  // ssl_dissector_add(SSL_PORT_SPDY, dissect_spdy);
...
void proto_reg_handoff_spdy(void) {
  ...
  heur_dissector_add("tcp", dissect_spdy_heur, "SPDY", "spdy", proto_spdy, TRUE);
}
